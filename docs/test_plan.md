...
这个测试计划只依赖仓库内的小文件、public MakeHuman demo、AI 示例图、轻量 CSE 输出和测试临时目录，
不要求提交 SMPL、DensePose 权重、公开数据集下载图片、CSE raw `.pt` 或任何私有数据集。
...
- `pytest tests/test_examples.py`：验证内置 AI 示例图片、轻量 CSE vertex maps、overlays 和 masks
  都存在且 vertex id 范围合法。
...
2. 导入 `examples/region_map.example.json` 或手工输入 `12, 55, 100-130`，确认 selection 只高亮，
   不会自动写入 region。
3. 加载 `examples/cse/vertex_maps/` 中任意一个 CSE `vertex_map.npz`。`Load CSE Map` 后，
   所有有效 vertex 应默认高亮；点击 `Add Selected` 后才写入当前 region。
4. 加载 `examples/images/` 中同名 CSE image，确认 2D overlay 与 3D selection 同步。
...
the public MakeHuman demo bundle, bundled AI examples, lightweight CSE maps, and
...
load a bundled CSE map from `examples/cse/vertex_maps/`, confirm that all valid
vertices are highlighted by default, then commit only with `Add Selected`. The
ribs, pelvis, and navel lines in 2D references should remain visual guides only
and must not affect exported vertex IDs.
